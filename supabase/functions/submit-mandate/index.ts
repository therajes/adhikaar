...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!representative) return json({ error: { code: 'active_credential_required' } }, 403)
    ...
    if (!policy) return json({ error: { code: 'active_policy_required' } }, 403)
    ...
    if (!requested.length || requested.some(action => !permitted.has(action))) return json({ error: { code: 'action_not_permitted' } }, 403)
    ...
    if (!validSignature) return json({ error: { code: 'invalid_signature' } }, 400)
    ...
    return json({ schemaVersion: 1, accepted: true, verificationCode: mandate.verificationCode }, 201)
  } catch {
    return json({ error: { code: 'invalid_request', message: 'The signed proof was not accepted.' } }, 400)
  }
})
