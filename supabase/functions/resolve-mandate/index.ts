...
Deno.serve(async (request) => {
  ...
  if (request.method !== 'POST') return json({ error: { code: 'not_available', message: 'The proof could not be resolved.' } }, 405)
  try {
    ...
    if (!await rateLimit(`resolve:${requester}`)) return json({ error: { code: 'try_later', message: 'Please wait before trying again.' } }, 429)
    const database = serviceClient()
    const { data: mandate } = await database.from('mandates').select('*').eq('verification_code', code).maybeSingle()
    if (!mandate) return json({ error: { code: 'not_available', message: 'The proof could not be resolved.' } }, 404)
    ...
    return json({ schemaVersion: 1, evidence: { mandate: mandate.mandate_json, signature: mandate.signature, canonicalHash: mandate.canonical_hash, representative, policy, institution, registrySnapshots: snapshots ?? [], revocations: revocations ?? [], safeCallback: callback, resolvedAt: new Date().toISOString() } })
  } catch {
    return json({ error: { code: 'invalid_request', message: 'The proof could not be resolved.' } }, 400)
  }
})
