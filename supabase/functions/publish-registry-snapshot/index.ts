...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!user) return json({ error: { code: 'authentication_required' } }, 401)
    const database = serviceClient()
    const { data: profile } = await database.from('profiles').select('platform_role').eq('id', user.id).single()
    if (!profile || !['validator','platform_admin'].includes(profile.platform_role)) return json({ error: { code: 'not_authorised' } }, 403)
    ...
    return json({ schemaVersion: 1, snapshot: data }, 201)
