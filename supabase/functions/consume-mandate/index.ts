...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!await rateLimit(`consume:${await sha256(sessionHash)}`, 8, 60)) return json({ error: { code: 'try_later' } }, 429)
    ...
    if (!mandate || mandate.citizen_challenge_digest !== challengeDigest || mandate.status !== 'active' || Date.parse(mandate.expires_at) <= Date.now()) return json({ error: { code: 'not_consumable' } }, 400)
    ...
    return json({ schemaVersion: 1, receipt: data })
  } catch {
    return json({ error: { code: 'invalid_request', message: 'This proof could not be consumed.' } }, 400)
  }
})
